...
const HeroSection = () => {
  return (
    <section className="min-h-screen flex flex-col justify-center pt-24 pb-12 bg-background bg-grid-pattern relative overflow-hidden">
      {/* Gradient orbs */}
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full">
          <div className="space-y-8 animate-fade-in-up">
            ...
            <p className="text-body-lg text-foreground/80 max-w-xl flex flex-wrap gap-x-3 gap-y-1">
              ...
              <span className="inline-flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-primary-hover animate-glow-pulse" style={{ animationDelay: '0.5s' }} />
                Operaciones automatizadas
              </span>
              <span className="inline-flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-accent-cyan animate-glow-pulse" style={{ animationDelay: '1s' }} />
          ...
          <div className="relative hidden lg:block animate-fade-in stagger-2">
            ...
            <div className="absolute -bottom-6 -left-6 w-32 h-32 bg-primary-hover/15 rounded-full blur-2xl animate-float" style={{ animationDelay: '1s' }} />
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-border mt-16 pt-12 relative z-10">
        <div className="container-bcr">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { end: 20, suffix: "+", label: "Empresas transformadas" },
              { end: 65, suffix: "%+", label: "Margen promedio" },
              { end: 12, suffix: " meses", label: "Contrato promedio" },
            ].map((stat, index) => (
              <div 
                key={stat.label}
                className="text-center p-6 rounded-xl bg-card border border-border hover:border-primary/30 transition-colors group"
              ...
              >
                <div className="stat-highlight inline-block">
                  <CountUp 
                    ...
                    suffix={stat.suffix} 
                    className="text-4xl lg:text-5xl font-bold text-primary group-hover:text-primary-hover transition-colors" 
                  />
                </div>
